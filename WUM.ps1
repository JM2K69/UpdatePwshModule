...
[System.Reflection.Assembly]::LoadFrom("$PSScriptRoot\assembly\MahApps.Metro.IconPacks.FontAwesome.dll") | out-null
...
Function New-Log {
    ...
    $logMessage | Out-File -Append -LiteralPath "$env:USERPROFILE\desktop\$verboseLogFile" 
}
...
$FindInstallModule.add_Click({
	New-Log "------------------------------------"
	New-Log "- Find PowerShell installed Module on $Env:COMPUTERNAME   -"
	New-Log "------------------------------------"
		...
		foreach($Value in $Global:ModulestoUpdateU)
			{
				...
				New-Log "Module $($Value.Name) is Add"
...
$InstallMModules.add_Click({
	...
	if ($AllowClobber.IsChecked -eq $true){
		...
		[MahApps.Metro.Controls.Dialogs.DialogManager]::ShowMessageAsync($Form, "Install Module", "The Module $Module is install with the version $Version with the option AllowClobber")
	...
	if ($AllowPrerelease.IsChecked -eq $true){
		...
		[MahApps.Metro.Controls.Dialogs.DialogManager]::ShowMessageAsync($Form, "Install Module", "The Module $Module is install with the version $Version with the option AllowPrerelease")
...
$Add_M.add_Click({
	...
	if ($DataGrid_Updated.SelectedItems.Count -gt 1)
	{
		...
		{
			new-log "working with $item "
			$test = [array]::IndexOf($Global:Cart,"$item")
			if ($test -eq "-1")
			{
				$Global:Cart.add($item)
				New-Log "The Module $item is add to the Cart"
...
$Global:Cart |  ForEach-Object {
	New-Log "Try to update the module $_"
	Install-Module -Name $_ -Force
	New-Log "The update is Finish for the module $_"
	$DataGrid_Updated.Items.Clear()
	$DataGrid_Updated.Items.Refresh()


}
$NBupdateModule.Content = "0"
})
